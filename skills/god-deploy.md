...
4. Record deploy evidence and any waiting artifact path in `.godpowers/state.json`.
   Run `npx godpowers state advance --step=deploy --status=done --project=.`
   when a tested real target or tested local staging harness exists. If
   deployed staging is deferred, leave the step `in-flight` unless local
   readiness is the verified scope and do not pause unless the user explicitly
   requested staging.
