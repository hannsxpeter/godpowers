...
const { existsArtifact } = require('./sync-fs');
...
function evaluateStateCondition(condition, projectRoot) {
  ...
  if (cond.startsWith('file:')) {
    // Probe .mdx then .md so old recipes naming legacy .md artifacts and old
    // projects holding legacy files both keep routing.
    return existsArtifact(projectRoot, cond.slice(5).trim());
  }
