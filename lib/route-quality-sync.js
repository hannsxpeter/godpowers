...
const TIER_GATE_COMMANDS = Object.freeze({
  '/god-prd': 'prd',
  '/god-design': 'design',
  '/god-arch': 'arch',
  '/god-roadmap': 'roadmap',
...
function detect(projectRoot) {
  ...
  for (const routePath of routes) {
    ...
    if (TIER_GATE_COMMANDS[command]) {
      const expectedGateCommand = `npx godpowers gate --tier=${TIER_GATE_COMMANDS[command]} --project=.`;
      const actualGateCommand = route.standards && route.standards['gate-command'];
      if (actualGateCommand !== expectedGateCommand) {
        missingGateCommandCount++;
        addCheck(
          checks,
          `missing-tier-gate-command-${command.replace(/[^a-z0-9]+/gi, '-')}`,
          'stale',
          routePath,
          `${command} must declare standards.gate-command as ${expectedGateCommand}.`,
          { spawn: 'god-auditor' }
        );
      }
    }
  ...
  addCheck(
    checks,
    'tier-gate-command-policy',
    missingGateCommandCount === 0 ? 'fresh' : 'stale',
    'routing/',
    missingGateCommandCount === 0
      ? 'All executable tier routes declare standards.gate-command.'
      : `${missingGateCommandCount} executable tier routes are missing standards.gate-command.`,
    { spawn: missingGateCommandCount === 0 ? null : 'god-auditor' }
  );
