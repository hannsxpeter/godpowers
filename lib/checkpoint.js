 ...
 * Checkpoint Manager
 *
 * Maintains `.godpowers/CHECKPOINT.mdx`, a human-readable + agent-readable
 ...
 *   - CHECKPOINT.mdx is the disk-authoritative "you are here" pin.
...
function checkpointPath(projectRoot) {
  return path.join(projectRoot, '.godpowers', 'CHECKPOINT.mdx');
}

// Pre-mdx runtimes wrote CHECKPOINT.md. Reads fall back to it; the first
// write absorbs it (its content round-trips through read()) and deletes it.
function legacyCheckpointPath(projectRoot) {
...
/**
 * Parse the checkpoint file (CHECKPOINT.mdx, legacy CHECKPOINT.md fallback).
 * Returns structured form or null if missing.
 */
function read(projectRoot) {
  let file = checkpointPath(projectRoot);
  if (!fs.existsSync(file)) {
    const legacy = legacyCheckpointPath(projectRoot);
...
/**
 * Write a fresh CHECKPOINT.mdx. Overwrites prior content and absorbs a
 * legacy CHECKPOINT.md twin left by a pre-mdx runtime.
 *
 * state shape:
