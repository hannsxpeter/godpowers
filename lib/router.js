...
function evaluateCheck(check, projectRoot) {
  ...
  if (check.startsWith('state:')) {
    ...
    if (expr === 'initialized') {
      return state.isInitialized(projectRoot);
    }
    const initializedMatch = expr.match(/^initialized\s*==\s*(.+)$/);
    if (initializedMatch) {
      return state.isInitialized(projectRoot) === parseValue(initializedMatch[1]);
    }
    ...
    if (!s) return false;
    const actual = readStatePath(s, dottedPath);
...
function readStatePath(currentState, dottedPath) {
  const rootValue = readDottedPath(currentState, dottedPath);
  if (rootValue !== undefined) return rootValue;
  return readDottedPath(currentState && currentState.tiers, dottedPath);
}

function readDottedPath(source, dottedPath) {
