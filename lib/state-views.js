...
// into the new .mdx view; the twin is deleted after any write so a project
// never holds two diverging views of the same state.
function planManagedWrite(filePath, body, opts) {
  ...
  return { next, unchanged, legacy, legacyExists };
}

function writeManaged(filePath, body, opts = {}) {
  const plan = planManagedWrite(filePath, body, opts);
  if (plan.unchanged) {
    if (plan.legacyExists) fs.unlinkSync(plan.legacy);
