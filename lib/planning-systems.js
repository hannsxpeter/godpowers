...
const { exists, existsArtifact, legacyTwin } = require('./sync-fs');
...
const SYSTEMS = {
  ...
  godaudits: {
    ...
    markerPaths: [siblingArtifacts.AUDIT_PATH],
    fileRoots: ['.godaudits'],
    standalonePatterns: []
  }
};

// Canonical sibling superskill artifacts. When one exists, detection is
// unambiguous: the file is agent-authored and machine-checked, so confidence
// is forced high regardless of how few files sit next to it.
const SIBLING_CANONICAL_MARKERS = [siblingArtifacts.PLAN_PATH, siblingArtifacts.AUDIT_PATH];
...
function classifyFile(filePath, content) {
  ...
  if (/\bGA-\d{3,}\b/.test(content) || RE_AUDIT_CHECK_ID.test(content) || RE_FINDING_HEADING.test(content)) {
    kinds.add('audit');
  }
...
function detectSystem(projectRoot, id, system) {
  ...
  const hasSiblingCanonical = markerHits.some((marker) => SIBLING_CANONICAL_MARKERS.includes(marker));
  const score = markerHits.length * 3 + fileRecords.length;
  const confidence = hasSiblingCanonical || score >= 10 || (hasPrimaryRoot && fileRecords.length >= 3)
    ? 'high'
    : (score >= 4 ? 'medium' : 'low');
  return {
    id,
...
function buildSourceLine(system, file) {
  ...
  return `- ${signalGrade(system.id)} ${system.name} source ${file.path} maps to ${file.kinds.join(', ')}${label}.`;
}
...
function buildImportedContext(detection, projectRoot) {
  ...
  // (.godplans/PLAN.mdx, .godaudits/AUDIT.mdx), recounted from their bodies.
  if (projectRoot) {
    lines.push('');
...
function buildSeedArtifact(detection, artifact) {
  ...
  for (const system of detection.systems) {
    ...
    for (const file of filesForKinds(system, artifact.sourceKinds).slice(0, 12)) {
      ...
      // Signals quote source lines verbatim, so GP-/GA-/R- ids survive into
      // the seed for later linkage.
      for (const signal of seedSignals(system, file)) {
        lines.push(`- ${grade} Imported signal from ${file.path}: ${signal}.`);
      }
      count += 1;
    }
  }
