...
|-------|-------|--------------|-----------|
| Single static binary (baseline) | 9.1 | Chosen. Meets the zero-runtime-dependency requirement with one cross-compile command. | |
| Shell script wrapping grep and awk | 6.4 | Rejected. PRD requires identical behavior on macOS and Linux, and BSD awk diverges from GNU awk on the parsing this needs. | |
| Language-server-style daemon with a thin client | 4.2 | Rejected. A daemon adds lifecycle, port, and stale-state failure modes to a tool that runs once per CI job and exits. | [HYPOTHESIS] Hidden cost: the daemon looks faster on repeat runs, but CI containers start cold every time, so the cache never warms. |

Scored against the NFR map below. Novelty is not a scoring axis.
