...
- [HYPOTHESIS] Godplans master plan contract found: [.godplans/PLAN.mdx plus validator status, legacy PLAN-only, or "none detected"].
...
- [DECISION] Sibling Godplans PLAN plus validator and canonical `.godaudits/AUDIT.json` artifacts stay owned by their products: import seeds from them, keep source and generated views read-only outside lifecycle-gated task execution, and write back only through `.godplans/GODPOWERS-SYNC.mdx` or `.godaudits/GODPOWERS-SYNC.mdx`.
