...
- [DECISION] Carve-out: facts read from `.godplans/PLAN.mdx` or `.godaudits/AUDIT.json` (GP/GA task status, `R-<DOM>-n` and `A-<DOM>-n` ids, compiled scores and coverage) are `[DECISION]`-grade citations of authored state when cited by id; product claims inferred beyond the plan stay `[HYPOTHESIS]`.
- [DECISION] `.godplans/` and `.godaudits/` artifacts are read-only for Godpowers except during explicit GP/GA task execution; a godaudits 2.x completion updates and validates AUDIT.json, then regenerates derived views, while all other write-back uses only `.godplans/GODPOWERS-SYNC.mdx` or `.godaudits/GODPOWERS-SYNC.mdx`.
