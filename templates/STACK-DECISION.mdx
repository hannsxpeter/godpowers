...
- **Does this category need to exist?**: [Yes, because X] or [No; folded into
  <other category>, and here is what that costs]
- **Candidates evaluated**: [Baseline: obvious choice], [Selected choice if
  different], [Alternative 1], [Alternative 2], ... (not capped at three; list
  what was actually considered)
- **Scores**: [Baseline] [N.N] / [Choice] [N.N] / [Alt1] [N.N] / [Alt2] [N.N]
- **Rejected, with reason**: [Alt1]: [specific reason tied to an ARCH NFR or
  ADR, not "less mature"]. [Alt2]: [HYPOTHESIS] Trap: [hidden cost; demoted,
  not deleted].
