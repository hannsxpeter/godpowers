...
The system shapes evaluated before the containers above were chosen. If exactly
one shape was considered, say so here and give the reason; do not leave the
section empty.

| Shape | Score | Why rejected | Trap flag |
|-------|-------|--------------|-----------|
| [Baseline: the obvious shape] | [N.N/10] | [Chosen, or the specific reason it lost] | |
...
Score against the NFR map and flip points below. Novelty is not a scoring axis.
