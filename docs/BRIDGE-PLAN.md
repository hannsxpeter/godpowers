...
- [DECISION] Remaining Phase 4 work: migrate root progress orchestration entry points and direct writer instructions to `state.json` and `godpowers state advance`, add remaining static decision-read checks, extend generated views beyond `PROGRESS.md`, update dogfood fixtures, and run the final 2.7.0 release path.
